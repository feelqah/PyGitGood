...
class GitRepository(object):
    ...
    def __init__(self, path, force=False):
        ...
        if not (force or os.path.isdir(self.gitdir)):
            raise Exception("Not a Git directory %s" % path)
...
def repo_create(path):
    """Ceate new repo at path"""

    repo = GitRepository(path, True)

    # Check if path doesn't exist or is empty dir
...
def object_read(repo, sha):
    ...
    with open(path, "rb") as f:
        ...
        if   fmt == b'commit' : c = GitCommit
            ...
            raise Exception("Unkown type %s for object %s".format(fmt.decode("ascii"), sha))
...
argsp = argsubparsers.add_parser("cat-file",
                                 help="Provide content of repo objects")
...
def ref_list(repo, path=None):
    ...
    #Git shows refs sorted. To do the same OrderedDict is used 
    #and the output of listdir is sorted
